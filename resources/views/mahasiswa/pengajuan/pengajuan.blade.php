    ...
    <form id="formPengajuan" method="POST" action="/academic-request" enctype="multipart/form-data">
      ...
      <div class="flex flex-col lg:flex-row lg:space-x-10 space-y-4 lg:space-y-0">
        ...
        <div class="space-y-2 text-left">
          ...
          <option value="Skripsi" {{ old('needs') == 'Skripsi' ? 'selected' : '' }}>Skripsi</option>
          <option value="Beasiswa" {{ old('needs') == 'Beasiswa' ? 'selected' : '' }}>Beasiswa</option>
          <option value="Aktif Kuliah" {{ old('needs') == 'Aktif Kuliah' ? 'selected' : '' }}>Aktif Kuliah
          </option>
          <option value="Lainnya" {{ old('needs') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
          </select>
        ...
        <div class="flex flex-col md:flex-row md:space-x-8 space-y-4 md:space-y-0">
          ...
          <div class="flex flex-col space-y-2">
            ...
            <input type="radio" name="language" value="indonesia" class="accent-orange-500" {{ old('language') == 'indonesia' ? 'checked' : '' }} required />
            <span>Bahasa Indonesia</span>
            </label>
            <label class="flex items-center space-x-2">
            <input type="radio" name="language" value="inggris" class="accent-orange-500" {{ old('language') == 'inggris' ? 'checked' : '' }} />
          ...
          <div class="flex flex-col space-y-2">
            <label class="flex items-center space-x-2">
            <input type="radio" name="signature_type" value="basah" class="accent-orange-500" {{ old('signature_type') == 'basah' ? 'checked' : '' }} required />
            <span>Basah</span>
            </label>
            <label class="flex items-center space-x-2">
            <input type="radio" name="signature_type" value="digital" class="accent-orange-500" {{ old('signature_type') == 'digital' ? 'checked' : '' }} />
            <span>Digital</span>
            </label>
          </div>
          </div>
...
@push('scripts')
  <script>
    ...
    quill.root.innerHTML = `{!! old('student_notes') !!}`;
    @endif
    ...
    @endif
    @if(old('needs'))
    document.getElementById('needs').value = "{{ old('needs') }}";
    if ("{{ old('needs') }}" === 'Lainnya') {
      document.getElementById('other_needs').classList.remove('hidden');
      document.getElementById('other_needs').required = true;
    }
    @endif
    @if(old('other_needs'))
    document.getElementById('other_needs').value = "{{ old('other_needs') }}";
    @endif
    @if(old('language'))
    document.querySelector(`input[name="language"][value="{{ old('language') }}"]`).checked = true;
    @endif
    @if(old('signature_type'))
    document.querySelector(`input[name="signature_type"][value="{{ old('signature_type') }}"]`).checked = true;
    @endif
